...
const UIDateInput = () => {
  ...
  useEffect(() => {
    const date = useStepsStore.getState().start_date;

    if (date) {
      const endDate_format_t = date.split('-');
      const endDate_format_spl = `${endDate_format_t[0]}.${endDate_format_t[1]}.${endDate_format_t[2]}`;
